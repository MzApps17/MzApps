  ...
  return (
    <main className="min-h-screen bg-[#f5f5f5] pb-28">
      ...
      <div className="p-3 flex flex-col gap-3">
        ...
        {/* ✅ THAR - ACCOUNT ACTIONS - Wishlist chung chiah a */}
        <div className="bg-white rounded-[26px] p-2 border border-gray-100 shadow-sm">
          <p className="text-[11px] font-black px-4 pt-2 pb-1 text-gray-400 tracking-widest">ACCOUNT ACTIONS</p>

          {!profile?.nameChanged? (
            <button onClick={openNameEdit} className="w-full flex items-center gap-3 px-3 py-3.5 text-left active:bg-gray-50 rounded-xl">
              <div className="w-11 h-11 bg-black rounded-full flex items-center justify-center text-white text-[18px]">✏️</div>
              <div className="flex-1"><p className="text-[14px] font-bold">Hming Thlak</p><p className="text-[12px] text-gray-400">Vawi 1 chiah thlak theih</p></div>
              <span className="text-gray-400">›</span>
            </button>
          ) : (
            <div className="flex items-center gap-3 px-3 py-3.5 opacity-50">
              <div className="w-11 h-11 bg-gray-200 rounded-full flex items-center justify-center">✏️</div>
              <div className="flex-1"><p className="text-[14px] font-bold">Hming Thlak</p><p className="text-[12px] text-gray-400">Vawi 1 i thlak tawh</p></div>
              <span className="text-[10px] bg-gray-100 px-2 py-1 rounded-full font-bold">Done</span>
            </div>
          )}
          ...
          {isAdmin && (
            <>
              <Link href="/admin/reports" className="w-full flex items-center gap-3 px-3 py-3.5 active:bg-gray-50 rounded-xl">
                <div className="w-11 h-11 bg-[#ffefef] rounded-full flex items-center justify-center text-[18px]">🚩</div>
                <div className="flex-1"><p className="text-[14px] font-bold text-red-600">Admin Reports</p><p className="text-[12px] text-gray-400">Report te enna</p></div>
                <span className="text-gray-400">›</span>
              </Link>
              <div className="h-[1px] bg-gray-100 mx-3"></div>
            </>
          ...
          <button onClick={async()=>{ await signOut(auth); router.push("/"); }} className="w-full flex items-center gap-3 px-3 py-3.5 text-left active:bg-red-50 rounded-xl">
            <div className="w-11 h-11 bg-[#ffefef] rounded-full flex items-center justify-center text-[18px]">🚪</div>
            <div className="flex-1"><p className="text-[14px] font-bold text-red-600">Log Out</p><p className="text-[12px] text-gray-400">Account chhuahsan</p></div>
            <span className="text-gray-400">›</span>
          </button>
        </div>
        ...
        <div className="bg-white rounded-[26px] p-4 border border-gray-100 shadow-sm">
          ...
          {wishlist.length===0? <p className="text-center text-gray-400 py-6 text-[13px]">Wishlist a la awm lo</p> : (
            <div className="grid grid-cols-2 gap-3">
              {wishlist.map((item: any) => (
                <div key={item.id} className="border rounded-[18px] overflow-hidden relative bg-gray-50">
                  <Link href={item.type === "job"? `/jobs/${item.id}` : `/marketplace/${item.id}`}>
                    <img src={item.image || item.images?.[0] || "/placeholder.png"} alt="" className="w-full h-[140px] object-cover" />
                  </Link>
                  <button onClick={() => removeWish(item.wishId, item.id)} className="absolute top-2 right-2 bg-black/70 text-white w-8 h-8 rounded-full flex items-center justify-center">✕</button>
                  <div className="p-2.5 bg-white"><p className="text-[13px] font-bold truncate">{item.title}</p></div>
                </div>
              ))}
            </div>
          )}
