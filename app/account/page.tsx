...
export default function Account(){
  ...
  const openPhoneEdit = ()=>{
    ...
    window.history.pushState({phoneModal:true},""); setShowPhoneEdit(true);
  };
  const handlePhoneSave = async()=>{
    ...
    try{
      ...
      if(window.history.state?.phoneModal) window.history.back();
    }catch(e:any){ alert(e.message); } finally{ setSavingPhone(false); }
  };

  // ✅ KHUA SAVE
  const openKhuaEdit = ()=>{
    setNewKhua(profile?.khua || profile?.village || profile?.location || "");
    window.history.pushState({khuaModal:true},""); setShowKhuaEdit(true);
  };
  const handleKhuaSave = async()=>{
    ...
    try{
      ...
      setShowKhuaEdit(false);
      if(window.history.state?.khuaModal) window.history.back();
  ...
  const openDobEdit = ()=>{
    ...
    window.history.pushState({dobModal:true},""); setShowDobEdit(true);
  };
  const handleDobSave = async()=>{
    ...
    try{
      ...
      if(window.history.state?.dobModal) window.history.back();
  ...
  return (
    <main className="min-h-screen bg-[#f7f7f8] pb-24">
      ...
      <div className="p-3 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-[24px] p-2 shadow-sm border border-gray-100">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-3 px-3 py-3.5">
              <div className="w-10 h-10 bg-[#f3f3f3] rounded-full flex items-center justify-center text-[16px]">✉️</div>
            ...
            <button onClick={openPhoneEdit} className="w-full flex items-center gap-3 px-3 py-3.5 active:bg-gray-50 rounded-xl text-left">
              <div className="w-10 h-10 bg-[#f3f3f3] rounded-full flex items-center justify-center text-[16px]">📱</div>
        ...
        {isAdmin && (
          <Link href="/admin/reports" className="w-full bg-[#E53935] text-white font-black py-4 rounded-[20px] flex items-center justify-center gap-2 shadow-lg shadow-red-200 active:scale-[0.98]">🚩 Admin - Reports En</Link>
        )}
        <button onClick={async()=>{ await signOut(auth); router.push("/"); }} className="w-full bg-white border border-red-100 text-red-600 font-black py-4 rounded-[20px] flex items-center justify-center gap-2 shadow-sm active:scale-[0.98]">↪ Log Out</button>
      </div>
      ...
      {showPic && displayPic && (
        <div className="fixed inset-0 bg-black/80 z-[999] flex items-center justify-center p-6 backdrop-blur-sm" onClick={closePic}>
          <div className="relative" onClick={e=>e.stopPropagation()}>
            ...
            <button onClick={closePic} className="absolute -top-3 -right-3 bg-white text-black w-9 h-9 rounded-full font-black shadow-lg">✕</button>
          </div>
        </div>
      )}
      ...
      {showNameEdit && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-6 backdrop-blur-sm">
          <div className="bg-white rounded-[22px] p-6 w-full max-w-[340px] shadow-2xl">
            ...
            <p className="text-[12px] text-red-500 mt-1 font-bold">⚠️ Vawi khat chiah!</p>
      ...
      {showPhoneEdit && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-6 backdrop-blur-sm">
          <div className="bg-white rounded-[22px] p-6 w-full max-w-[340px] shadow-2xl">
            ...
            <p className="text-[12px] text-gray-400 mt-1">I phone number dah rawh</p>
      ...
      {showKhuaEdit && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-6 backdrop-blur-sm">
          <div className="bg-white rounded-[22px] p-6 w-full max-w-[340px] shadow-2xl">
            ...
            <p className="text-[12px] text-gray-400 mt-1">I awmna khua dah rawh</p>
            ...
            <div className="flex gap-2 mt-5">
              <button onClick={()=>{ setShowKhuaEdit(false); i
