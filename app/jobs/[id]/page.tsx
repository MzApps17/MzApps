...
import { doc, getDoc, collection, getDocs, setDoc, deleteDoc, serverTimestamp, query, where, limit } from "firebase/firestore";
...
export default function JobDetail(){
  ...
  useEffect(()=>{
    const getJob=async()=>{
      ...
      if(snap.exists()){
        ...
        // ✅ RELATED - CATEGORY IN ANG
        try{
          const cat = (data as any).category || (data as any).jobCategory || "";
          let rel:any[] = [];
          if(cat){
            const q = query(collection(db,"jobs"), where("category","==",cat), limit(10));
            const qs = await getDocs(q);
            rel = qs.docs.map(d=>({id:d.id,...d.data()})).filter((p:any)=>p.id!==id);
            if(rel.length===0){
              const q2 = query(collection(db,"jobPosts"), where("category","==",cat), limit(10));
              const qs2 = await getDocs(q2);
              rel = qs2.docs.map(d=>({id:d.id,...d.data()})).filter((p:any)=>p.id!==id);
            }
          }
          if(rel.length===0){
            const dist = (data as any).district || "";
            if(dist){
              const q3 = query(collection(db,"jobs"), where("district","==",dist), limit(10));
              const qs3 = await getDocs(q3);
              rel = qs3.docs.map(d=>({id:d.id,...d.data()})).filter((p:any)=>p.id!==id);
            }
          }
        ...
        if(typeof d.likeCount==='number') lc=d.likeCount;
        else if(typeof d.likesCount==='number') lc=d.likesCount;
        else if(Array.isArray(d.likes)) lc=d.likes.length;
        try{
          ...
          else if(likeSnap2.size>0) lc = likeSnap2.size;
          const u = auth.currentUser;
          if(u){
            const c1 = await getDoc(doc(db,"jobs",id as string,"likes",u.uid));
            const c2 = await getDoc(doc(db,"jobPosts",id as string,"likes",u.uid));
            if(c1.exists()||c2.exists()) setLiked(true);
          }
        }catch{}
        if(auth.currentUser && Array.isArray(d.likes) && d.likes.includes(auth.currentUser.uid)) setLiked(true);
        setLikeCount(lc);
        try{
        ...
        }catch{ setCommentCount(d.commentCount||0); }
      }
    };
    if(id) getJob();
  },[id]);
  ...
  const toggleWish=async()=>{
    ...
    else{ await setDoc(wishRef,{productId:id, jobId:id, type:"job", createdAt:serverTimestamp()}); setWished(true); }
  };

  const sendNoti = async(type:"like"|"comment")=>{
    try{
      ...
      await setDoc(notiRef,{
        type, postId:id, postType:"job",
        fromUid:u.uid, fromName:u.displayName||u.email?.split("@")[0]||"Someone",
        fromPhoto:u.photoURL||"", postTitle:job.title||job.jobTitle||"your job",
        message:type==="like"? "liked your job":"commented on your job",
  ...
  const toggleLike = async()=>{
    ...
    if(liked){
      ...
      try{ await deleteDoc(likeRef); await deleteDoc(likeRef2); }catch{}
    }else{
      setLiked(true); setLikeCount(c=>c+1);
      try{ await setDoc(likeRef,{userId:u.uid,createdAt:new Date()}); await setDoc(likeRef2,{userId:u.uid,createdAt:new Date()}); }catch{}
      sendNoti("like");
    }
  };
  ...
  const handleShare = async()=>{
    ...
    try{
      if(navigator.share) await navigator.share({title: job?.title||job?.jobTitle, url});
  ...
  return (
    <main className="bg-white min-h-screen pb-[140px]">
      <div className="flex items-center justify-between p-3 pt-4 bg-white sticky top-0 z-50">
        ...
        <button onClick={toggleWish} className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-lg border text-[22px]">{wished? "❤️":"🤍"}</button>
      </div>
      <div className="p-4 pt-2">
        ...
        <div className="flex items-center gap-2.5 mt-4">
          <button onClick={toggleLike} className={`flex items-center gap-2 px-5 py-2.5 rounded-full border-2 font-black text-[14px] ${liked?"bg-black text-white border-black":"bg-white border-gray-200 text-black"}`}><span>{liked?"❤️":"🤍"}</span> {likeCount}</button>
          <button onClick={()=>setShowComment(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-full border-2 bg-white border-gray-200 font-black text-[14px] text-black"><span>💬</span> {commentCount}</button>
          <button onClick={handleShare} className="flex items-center gap-2 px-5 py-2.5 rounded-full border-2 bg-white border-gray-200 font-black text-[14px] text-black"><span>↗️</span> Share</button>
        </div>
        ...
        <div onClick={()=>{ if(sellerUid) router.push(`/seller/${sellerUid}`); }} className="bg-[#f8f9fa] rounded-2xl p-4 mt-4 flex items-center gap-3 cursor-pointer">
          <div className="w-[56px] h-[56px] bg-black text-white rounded-full flex items-center justify-center font-black overflow-hidden flex-shrink-0">{displayPic? <img src={displayPic} className="w-full h-full object-cover"/> : <span>{displayName[0]?.toUpperCase()}</span>}</div>
          <div className="flex-1"><p className="font-bold text-[17px] capitalize">{displayName}</p><p className="text-[13px] text-gray-500">Verified Employer • View Profile</p></div>
        ...
        {related.length>0 && (
          <div className="mt-5">
            ...
            <div className="grid grid-cols-1 gap-3">
              {related.map((p:any)=>(<Link key={p.id} href={`/jobs/${p.id}`} className="bg-[#f8f9fa] border border-gray-100 rounded-[18px] p-4"><p className="font-bold text-[14px]">{p.title||p.jobTitle}</p><p className="font-black text-[14px] mt-1 text-[#008a3d]">₹{typeof p.salary==='number'? Number(p.salary).toLocaleString("en-IN"):p.salary}/month</p><p className="text-[12px] text-gray-500 mt-1">{p.location||p.village} • {p.company||""}</p></Link>))}
            </div>
          </div>
        )}
      </div>
      <div className="fixed bottom-[60px] left-0 right-0 bg-white border-t border-gray-200 p-3 flex gap-3 z-[100]">
        <a href={waLink} target="_blank" className="flex-1 border-2 border-black rounded-xl py-3.5 font-black text-[16px] flex items-center justify-center gap-2 bg-white text-black">WhatsApp</a>
      ...
      {showComment && <CommentPopup postId={id as string} onClose={()=>setShowComment(false)} onCommentAdded={()=>{ setCommentCount(c=>c+1); sendNoti("comment"); }} />}
    </main>
  );
            }
