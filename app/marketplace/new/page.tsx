...
export default function NewProduct(){
  ...
  const submit=async(e:any)=>{
    ...
    try {
      ...
      if(!key) throw new Error("IMGBB Key hmuh loh - Vercel ah dah rawh");
      ...
      await addDoc(collection(db,"products"),{title,price:Number(price),image:imageUrl,imageUrl,uid:user.uid,createdAt:serverTimestamp()});
      alert("Post hlawhtling!");
      router.push("/marketplace");
    } catch(err:any){ alert("Error: " + err.message); }
    finally{ setLoading(false); }
  }
  return <form onSubmit={submit} className="p-4 max-w-md mx-auto flex flex-col gap-3"><h1 className="font-bold text-xl">Thil Zuarh</h1><input required value={title} onChange={e=>setTitle(e.target.value)} placeholder="Bike" className="border p-3 rounded-xl"/><input required value={price} onChange={e=>setPrice(e.target.value)} placeholder="20000" type="number" className="border p-3 rounded-xl"/><input required type="file" accept="image/*" onChange={e=>setFile(e.target.files?.[0]||null)} className="border p-3 rounded-xl"/><button disabled={loading} className="bg-blue-600 text-white p-3 rounded-xl font-bold">{loading?"Uploading...":"Post rawh"}</button></form>
}
