  ...
  return (
    <main className="bg-white min-h-screen pb-[140px]">
      ...
      <div className="relative bg-black w-full aspect-[4/3] overflow-hidden">
        <div className="w-full h-full flex transition-transform duration-300" style={{transform:`translateX(-${currentImg*100}%)`}}>
          {allImages.map((img:string,i:number)=>(
            <img key={i} src={img} className="w-full h-full object-contain flex-shrink-0 bg-black"/>
          ))}
        </div>

        {/* ARROW LIAN - Create Post ang chiah SVG 30 */}
        ...
        {/* DOTS - Pic tam chuan a lang */}
        {allImages.length > 1 && (
          <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5">
            {allImages.map((_:any,i:number)=>(
              <button key={i} onClick={()=>setCurrentImg(i)} className={`h-2 rounded-full transition-all ${i===currentImg?"w-6 bg-white":"w-2 bg-white/50"}`}/>
            ))}
          </div>
        )}
        ...
        {allImages.length > 1 && (
          <>
            <button onClick={()=>setCurrentImg(p=>p===0?allImages.length-1:p-1)} className="absolute left-0 top-0 bottom-0 w-[25%]"/>
            <button onClick={()=>setCurrentImg(p=>p===allImages.length-1?0:p+1)} className="absolute right-0 top-0 bottom-0 w-[25%]"/>
          </>
        )}
      </div>
      ...
      <div className="p-4">
        ...
        <div className="bg-[#f8f9fa] rounded-2xl p-4 mt-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-bold text-[14px]">{product.userEmail?.split("@")[0] || "nghaka699"}</p>
            <p className="text-[12px] text-gray-500">Verified Seller</p>
          </div>
        </div>

        {/* THLALAK TAM CHUAN THUMBNAIL LANG RAWH SE */}
        {allImages.length > 1 && (
          <div className="mt-4">
            <p className="font-black text-[13px] mb-2">Photos ({allImages.length})</p>
            <div className="flex gap-2 overflow-x-auto pb-2">
              {allImages.map((img:string,i:number)=>(
                <img key={i} onClick={()=>setCurrentImg(i)} src={img} className={`w-20 h-20 rounded-xl object-cover border-2 flex-shrink-0 cursor-pointer ${i===currentImg?"border-black":"border-transparent"}`}/>
              ))}
            </div>
          </div>
        )}
      ...
      {/* BOTTOM BUTTONS - A LANG THEI LO KHA FIX */}
