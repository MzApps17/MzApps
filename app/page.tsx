...
export default function LoginPage() {
  ...
  const handleSend = async()=>{
    ...
    try{
      ...
      if(res.ok){
        setStep("otp");
        setAlertMsg(`OTP thawn a ni! ${email} ah en rawh - 30 mins valid!`);
      } else {
        const txt = await res.text();
        setAlertMsg("Email thawn theih loh: " + txt + " - Public Key check rawh");
      }
  ...
  const handleVerify = async()=>{
    ...
    if(diff > 30*60*1000){ setAlertMsg("OTP expired (30 min pel tawh)"); setLoading(false); return; }
