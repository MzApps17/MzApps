    ...
    <main className="min-h-screen bg-[#f2f2f2]">
      ...
      <div className="bg-white sticky top-0 z-20 p-3 border-b">
        ...
        <div className="flex gap-2 overflow-x-auto mt-3 pb-1 no-scrollbar">
          {categories.map(c=>(
            <button key={c} onClick={()=>setCat(c)} className={`whitespace-nowrap px-4 py-1.5 rounded-full border text-[13px] font-black transition-all ${cat===c?'bg-[#002f34] text-white border-[#002f34] scale-105':'bg-white text-[#002f34] border-gray-300'}`}>{c}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-[1px] bg-gray-200">
        {filtered.map(ad=>(
          <Link key={ad.id} href={`/marketplace/${ad.id}`} className="bg-white p-2 flex flex-col group">
            <div className="w-full h-40 bg-gray-100 overflow-hidden">
              <img
                src={ad.image || ad.images?.[0] || "https://via.placeholder.com/300"}
                alt={ad.title}
                loading="lazy"
      ...
      {loading && (
        <div className="grid grid-cols-2 gap-[1px] bg-gray-200">
          {[1,2,3,4,5,6].map(i=><div key={i} className="bg-white h-56 animate-pulse"/>)}
        </div>
      )}

      {isLoadingMore && <p className="text-center py-4 text-[12px] font-bold text-gray-400">Loading more...</p>}
      {!hasMore &&!loading && <p className="text-center py-4 text-[12px] font-bold text-gray-400">A zo vek e!</p>}
    </main>
  );
}
