// app/admin/reports/page.tsx - FINAL - 7 USERS LANG VEK TUR
"use client";
...
export default function ReportsPage(){
  ...
  // THAR - FIXED - orderBy awm lo, 7 vek a la ang
  const fetchAllUsers = async () => {
    try{
      ...
      const list = snap.docs.map(d=> {
        ...
        return {
          id: d.id,
          displayName: data.displayName || data.name || data.email?.split('@')[0] || "Hming awm lo",
        ...
        <div className="flex flex-col gap-3 mt-2">
          {reports.map(r=>(
            <div key={r.id} className="border-2 rounded-2xl p-4 bg-white">
              <div className="flex justify-between"><p className="font-black text-[16px]">{r.reportedUser?.displayName || r.reportedUser?.name || r.sellerName || "Hming awm lo"}</p><p className="text-[11px] bg-red-50 text-red-600 px-2 py-1 rounded-full h-fit">{r.createdAt?.toDate? r.createdAt.toDate().toLocaleDateString() : "new"}</p></div>
              <p className="text-[13px] mt-2 bg-gray-50 p-3 rounded-xl">"{r.message}"</p>
              <div className="mt-3 bg-gray-50 p-2.5 rounded-xl space-y-1"><p className="text-[12px]"><span className="text-gray-500">🚩 Reported:</span> <span className="font-black text-black">{r.reportedUser?.displayName || r.reportedUser?.name || r.sellerName || "Unknown"}</span></p><p className="text-[12px]"><span className="text-gray-500">👤 Reporter:</span> <span className="font-black text-black">{r.reporterUser?.displayName || r.reporterUser?.name || "Anonymous"}</span></p></div>
              <div className="grid grid-cols-3 gap-2 mt-3"><button onClick={()=>setConfirmModal({type:'post', reportId:r.id, userId:r.reportedUserId, userName: r.reportedUser?.displayName || "User"})} disabled={actionId===r.id} className="bg-orange-500 text-white py-2.5 rounded-xl text-[11px] font-black active:scale-95 disabled:opacity-50">{actionId===r.id? "..." : "🗑️ POST DELETE"}</button><button onClick={()=>setConfirmModal({type:'ban', reportId:r.id, userId:r.reportedUserId, userName: r.reportedUser?.displayName || "User"})} disabled={actionId===r.id} className="bg-red-600 text-white py-2.5 rounded-xl text-[11px] font-black active:scale-95 disabled:opacity-50">{actionId===r.id? "..." : "🚫 BAN USER"}</button><button onClick={()=>setShowDeleteId(r.id)} className="bg-gray-200 py-2.5 rounded-xl text-[11px] font-bold">Delete Report</button></div>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-col gap-2 mt-4 max-h-[600px] overflow-y-auto">
            {filteredUsers.map(u=>(
              <div key={u.id} className="border rounded-2xl p-3 flex gap-3 bg-white">
                ...
                <div className="flex-1 min-w-0"><p className="font-black text-[13px] truncate">{u.displayName || u.name || "Hming awm lo"}</p><p className="text-[11px] text-gray-600 truncate">{u.email || "Email awm lo"}</p><p className="text-[10px] text-gray-400">ID: {u.id.slice(0,12)}... • {u.bio || ""}</p></div>
