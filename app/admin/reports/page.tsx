// app/admin/reports/page.tsx - FIXED - Hming awm lo / Email awm lo a bo tawh ang
"use client";
...
// ✅ HELPER - Email atangin hming siam
...
export default function ReportsPage(){
    ...
    const unsub=onAuthStateChanged(auth, async(u)=>{
      ...
      try {
        ...
        for(const d of snap.docs){
          ...
          // ✅ FIX: users doc a awm loh pawhin report atangin emaw post atangin hming zawng chhuak rawh
          if(!reportedUser || (!reportedUser.displayName &&!reportedUser.email)){
            try{
              // A post atangin email / hming la chhuak tum rawh
  ...
  // ✅ FIXED - Email awm lo te pawh Post atangin a la ang
  const fetchAllUsers = async () => {
    try{
      ...
      let list = snap.docs.map(d=> {
        ...
        return {
          id: d.id,
          displayName: name || data.email?.split('@')[0] || "Hming awm lo",
      ...
      // A la ruak te tan Post atangin zawng chhuak leh rawh
      for(let i=0; i<list.length; i++){
        const u = list[i] as any;
        if(!u.email || u.displayName==="Hming awm lo" ||!u._rawName){
          try{
            const pq = query(collection(db,"products"), where("userId","==", u.id), limit(1));
            const psnap = await getDocs(pq);
            if(!psnap.empty){
              const pd = psnap.docs[0].data() as any;
              if(!u.email && getEmailFromData(pd)){
                u.email = getEmailFromData(pd);
              }
              if(u.displayName==="Hming awm lo" ||!u._rawName){
                const n = getNameFromData(pd);
                if(n) u.displayName = n;
              }
            }
          }catch{}
          // Email atangin hming siam leh rawh
          if((!u.displayName || u.displayName==="Hming awm lo") && u.email){
            u.displayName = u.email.split('@')[0];
          }
          // A tawp ber - ID hmangin
    ...
    <main className="min-h-screen bg-white pb-10">
      ...
      <div className="p-3 max-w-md mx-auto">
        ...
        <div className="flex flex-col gap-3 mt-2">
          {reports.map(r=>{
            const rName = getNameFromData(r.reportedUser) || r.sellerName || r.reportedUser?.email?.split('@')[0] || r.reportedUserId?.slice(0,8) || "Hming awm lo";
            const rEmail = getEmailFromData(r.reportedUser) || r.reportedUser?.email || "";
            return (
            <div key={r.id} className="border-2 rounded-2xl p-4 bg-white">
              ...
              <div className="grid grid-cols-3 gap-2 mt-3"><button onClick={()=>setConfirmModal({type:'post', reportId:r.id, userId:r.reportedUserId, userName: rName})} disabled={actionId===r.id} className="bg-orange-500 text-white py-2.5 rounded-xl text-[11px] font-black active:scale-95 disabled:opacity-50">{actionId===r.id? "..." : "🗑️ POST DELETE"}</button><button onClick={()=>setConfirmModal({type:'ban', reportId:r.id, userId:r.reportedUserId, userName: rName})} disabled={actionId===r.id} className="bg-red-600 text-white py-2.5 rounded-xl text-[11px] font-black active:scale-95 disabled:opacity-50">{actionId===r.id? "..." : "🚫 BAN USER"}</button><button onClick={()=>setShowDeleteId(r.id)} className="bg-gray-200 py-2.5 rounded-xl text-[11px] font-bold">Delete Report</button></div>
            </div>
          )})}
        </div>
          ...
          <div className="flex flex-col gap-2 mt-4">
            {filteredPosts.map(p=>(
              <div key={p.id} className="border rounded-2xl p-3 flex gap-3 bg-gray-50">
                ...
                <div className="flex-1 min-w-0"><p className="font-black text-[13px] truncate">{p.title || p.name || "No title"}</p><p className="text-[12px] font-bold text-green-600">₹{p.price || 0} • {p.col}</p><p className="text-[10px] text-gray-400 truncate">{p.userId?.slice(0,10)}... • {p.createdAt?.toDate?.()?.toLocaleDateString() || ""}</p><div className="flex gap-2 mt-2"><button onClick={()=>setEditPost(p)} className="bg-black text-white px-3 py-1.5 rounded-full text-[10px] font-black">✏️ EDIT</button><button onClick={()=>deleteSinglePost(p)} className="bg-red-600 text-white px-3 py-1.5 rounded-full text-[10px] font-black">🗑️ DELETE</button></div></div>
              </div>
            ))}
        ...
        <div className="mt-10 border-t-4 border-black pt-6">
          ...
          <p className="text-[12px] text-gray-500 font-bold mb-3">Login tawh zawng zawng - i screenshot a 7 awm ho kha!</p>
          ...
          <div className="flex flex-col gap-2 mt-4 max-h-[600px] overflow-y-auto">
            {filteredUsers.map(u=>{
              const dName = u.displayName || getNameFromData(u) || u.id.slice(0,8);
              const dEmail = u.email || getEmailFromData(u) || "Email awm lo";
              return (
              <div key={u.id} className="border rounded-2xl p-3 flex gap-3 bg-white">
                ...
                <div className="flex-1 min-w-0"><p className="font-black text-[13px] truncate">{dName}</p><p className="text-[11px] text-gray-600 truncate">{dEmail}</p><p className="text-[10px] text-gray-400">ID: {u.id.slice(0,12)}... • {u.bio || ""}</p></div>
                <button onClick={()=>setConfirmDeleteUser({id:u.id, name:dName})} className="bg-red-600 text-white px-4 h-fit py-2 rounded-full text-[10px] font-black active:scale-95">🗑️ DELETE</button>
              </div>
            )})}
            {filteredUsers.length===0 && <p className="text-center text-gray-400 font-bold mt-10">User hmuh loh - Loading...</p>}
          </div>
        </div>
      </div>
      ...
      {confirmModal && (<div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-6 backdrop-blur-sm"><div className="bg-white rounded-[28px] p-6 w-full max-w-[340px] shadow-2xl text-center"><div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3 text-2xl ${confirmModal.type==='ban'? 'bg-red-100' : 'bg-orange-100'}`}>{confirmModal.type==='ban'? '🚫' : '🗑️'}</div><p className="font-black text-[18px] text-[#002f34] leading-ti
