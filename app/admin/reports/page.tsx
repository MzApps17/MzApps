// app/admin/reports/page.tsx - FINAL
"use client";
...
export default function ReportsPage(){
  ...
  const handleBanUser = async (reportedUserId: string, reportId: string) => {
    if(!confirm("He User hi BAN / DELETE vek ang em?")) return;
    setActionId(reportId);
    try{
          ...
          {reports.map(r=>(
            <div key={r.id} className="border-2 rounded-2xl p-4 bg-white">
              ...
              <div className="grid grid-cols-3 gap-2 mt-3">
                <button onClick={()=>handleDeletePosts(r.reportedUserId, r.id)} disabled={actionId===r.id} className="bg-orange-500 text-white py-2.5 rounded-xl text-[11px] font-black active:scale-95 disabled:opacity-50">{actionId===r.id? "..." : "🗑️ POST DELETE"}</button>
                <button onClick={()=>handleBanUser(r.reportedUserId, r.id)} disabled={actionId===r.id} className="bg-red-600 text-white py-2.5 rounded-xl text-[11px] font-black active:scale-95 disabled:opacity-50">{actionId===r.id? "..." : "🚫 BAN USER"}</button>
