// app/admin/reports/page.tsx - REPORTS + ALL POSTS + ALL USERS MANAGER
"use client";
...
export default function ReportsPage(){
  ...
  const fetchAllUsers = async () => {
    try{
      ...
      // orderBy fail chuan orderBy lo in
  ...
  const executeDeleteUserAccount = async () => {
    ...
    try{
      ...
      // a post zawng zawng delete tel duh chuan
      ...
      setSuccessModal(`${confirmDeleteUser.name} account delete fel! 🗑️`);
      fetchAllPosts();
    }catch(e:any){ setSuccessModal("Error: "+e.message); }
    setDeleting(false);
          ...
          <div className="flex flex-col gap-2 mt-4 max-h-[600px] overflow-y-auto">
            {filteredUsers.map(u=>(
              <div key={u.id} className="border rounded-2xl p-3 flex gap-3 bg-white">
                <img src={u.photoURL || "/no-image.png"} className="w-12 h-12 rounded-full object-cover bg-gray-100 border" />
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-400">ID: {u.id.slice(0,12)}... • {u.createdAt?.toDate?.()?.toLocaleDateString() || "new"}</p>
                </div>
            ...
            {filteredUsers.length===0 && <p className="text-center text-gray-400 font-bold mt-10">User hmuh loh / la awm lo</p>}
          </div>
        </div>
      </div>
      ...
      {confirmDeleteUser && (
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-6 backdrop-blur-sm">
          <div className="bg-white rounded-[28px] p-6 w-full max-w-[340px] shadow-2xl text-center">
            ...
            <p className="font-black text-[18px] text-[#002f34] leading-tight">Delete {confirmDeleteUser.name}?</p>
            <p className="text-[12px] text-gray-500 mt-2">A account + a post zawng zawng a bo vek ang! A kir leh thei lo!</p>
            <div className="flex gap-2 mt-6">
              <button onClick={()=>setConfirmDeleteUser(null)} className="flex-1 bg-[#f3f4f6] text-black font-bold py-3.5 rounded-2xl text-[14px] active:scale-95">Cancel</button>
              <button onClick={executeDeleteUserAccount} disabled={deleting} className="flex-1 bg-red-600 text-white font-black py-3.5 rounded-2xl text-[14px] active:scale-95">{deleting?"...":"DELETE"}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 z-[1000] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[28px] p-6 w-full max-w-[360px] shadow-2xl">
            ...
            <div className="flex flex-col gap-3">
              <input value={editPost.title || ""} onChange={e=>setEditPost({...editPost, title:e.target.value})} placeholde
