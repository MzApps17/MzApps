...
export default function Login(){
  ...
  const showAlert=(msg:string)=>{
    if(msg.toLowerCase().includes("invalid-email")){ setAlertMsg("i Email format a dik lo"); }
  ...
  const resetPass=async()=>{
    ...
    try{
      ...
      showAlert("Password Reset Link i email ah ka thawn e! - "+email);
      ...
      <div className="flex-1 flex items-start justify-center p-4 pt-3">
        <form onSubmit={submit} className="bg-white w-full max-w-sm rounded-[28px] p-7 border shadow-sm">
          ...
          <div className="relative mt-3">
            <input value={pass} onChange={e=>setPass(e.target.value)} placeholder="Password" type={showPass?"text":"password"} className="w-full border-2 border-gray-200 p-4 pr-12 rounded-2xl outline-none focus:border-black focus:ring-1 focus:ring-black" required />
            <button type="button" onClick={()=>setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2">
              {showPass? <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/><line x1="2" y1="2" x2="22" y2="22" strokeWidth="2.5" strokeLinecap="round"/></svg> : <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>}
            </button>
          </div>
          ...
          <button type="button" onClick={googleLogin} className="w-full border-2 border-gray-200 p-4 rounded-2xl font-bold flex items-center justify-center gap-2">
          ...
          <div className="w-full mt-4 text-sm text-gray-600 text-center">
            ...
            <button type="button" onClick={()=>setIsSignup(!isSignup)} className="font-black text-blue-600">{isSignup?"Log In":"Sign Up"}</button>
          </div>
        </form>
      </div>
      ...
      {alertMsg && (
        <div className="absolute inset-0 bg-black/50 flex items-center justify-center p-6 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-xs text-center">
            <p className="text-[15px] font-bold">{alertMsg}</p>
