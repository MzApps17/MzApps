...
export default function ProfilePage(){
  ...
  const btnBg = darkMode? '#222' : '#efefef'

  return <div style={{minHeight:'100vh', background:bg, color:text, paddingBottom:80}}>
    <style>{`.mz-input:focus{outline:2px solid #7C3AED!important;border-color:#7C3AED!important;} *{box-sizing:border-box}`}</style>

    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 12px', borderBottom:`1px solid ${border}`, position:'sticky', top:0, background:bg2, zIndex:20, color:text}}>
      <span style={{fontWeight:900, fontSize:22}}>{name}</span>
      <button onClick={()=>setShowMenu(!showMenu)} style={{background:'none', border:'none', fontSize:28, lineHeight:1, color:text}}>☰</button>
    </div>

    {showMenu && <><div onClick={()=>setShowMenu(false)} style={{position:'fixed', inset:0, zIndex:40}}/>
    <div style={{position:'fixed', top:44, right:10, width:180, background:bg2, borderRadius:16, boxShadow:'0 8px 30px rgba(0,0,0,0.15)', zIndex:50, overflow:'hidden', border:`1px solid ${border}`}}>
      <button onClick={()=>{setShowMenu(false); router.push('/settings')}} style={{width:'100%', display:'flex', alignItems:'center', gap:10, padding:'14px 16px', border:'none', background:bg2, fontSize:16, fontWeight:500, cursor:'pointer', color:text}}>
      ...
      <button onClick={()=>{localStorage.clear(); router.push('/')}} style={{width:'100%', display:'flex', alignItems:'center', gap:10, padding:'14px 16px', border:'none', background:bg2, fontSize:16, fontWeight:600, color:'#ff3b30', cursor:'pointer'}}>
    ...
    <div style={{padding:12}}>
      <div style={{display:'flex', gap:10, alignItems:'flex-start'}}>
        <div style={{position:'relative'}}>
          <div onClick={()=>setShowPicView(true)} style={{width:86, height:86, borderRadius:'50%', background: pic?`url(${pic}) center/cover`:'#ddd', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:900, fontSize:32, cursor:'pointer', color:'#000'}}>{!pic && name[0]}</div>
      ...
      <div style={{marginTop:42, display:'flex', flexDirection:'column', gap:12}}>
        ...
        <div style={{borderBottom:`1px solid ${border}`, margin:'0 0 0 32px'}}></div>
      ...
      <div style={{display:'flex', gap:8, marginTop:28}}>
        <button onClick={()=>setShowEdit(true)} style={{flex:1, padding:'11px', borderRadius:10, background:btnBg, border:'none', fontWeight:800, fontSize:15, color:text}}>Edit profile</button>
        <button onClick={async()=>{const link=`https://mz-apps-mauve.vercel.app/user/${auth.currentUser?.uid}`; if(navigator.share) await navigator.share({title:name, url:link}); else {await navigator.clipboard.writeText(link); alert('Copied!')}}} style={{flex:1, padding:'11px', borderRadius:10, background:btnBg, border:'none', fontWeight:800, fontSize:15, color:text}}>Share profile</button>
      </div>
    </div>

    <div ref={postsRef} style={{marginTop:26}}>
      <div style={{display:'flex', justifyContent:'center', padding:'14px 0'}}><span style={{fontWeight:900, borderBottom:`2px solid ${text}`, paddingBottom:4, fontSize:18}}>⊞ POSTS</span></div>
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:2}}>
        {posts.length===0? [1,2,3].map(i=><div key={i} style={{aspectRatio:'1', background: darkMode? '#1a1a1a':'#fafafa', display:'flex', alignItems:'center', justifyContent:'center', color:'#888', fontSize:14}}>No posts</div>) : posts.map((p,i)=><div key={i} style={{aspectRatio:'1', background:`url(${p.image}) center/cover #eee`}}/>)}
      </div>
    </div>
    ...
    {showEdit && <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:90, display:'flex', alignItems:'center', justifyContent:'center', padding:16, overflowY:'auto'}}>
      <div style={{background:bg2, color:text, borderRadius:20, padding:18, width:'100%', maxWidth:380, maxHeight:'90vh', overflowY:'auto'}}>
        <h3 style={{fontWeight:900, textAlign:'center'}}>Edit Profile</h3>
        <div style={{display:'flex', flexDirection:'column', gap:10, marginTop:12}}>
          <input className="mz-input" value={editData.name} onChange={e=>setEditData({...editData,name:e.target.value})} placeholder="Name" style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          <textarea className="mz-input" value={editData.bio} onChange={e=>setEditData({...editData,bio:e.target.value})} placeholder="Bio" style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          <div style={{width:'100%'}}>
            <div style={{fontSize:12, fontWeight:700, marginBottom:4, color:'#888'}}>DOB</div>
            <input className="mz-input" type="date" value={editData.dob} onChange={e=>setEditData({...editData,dob:e.target.value})} style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          </div>
          <input className="mz-input" value={editData.hobby} onChange={e=>setEditData({...editData,hobby:e.target.value})} placeholder="Hobby" style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          <input className="mz-input" value={editData.games} onChange={e=>setEditData({...editData,games:e.target.value})} placeholder="Games" style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          <input className="mz-input" value={editData.country} onChange={e=>setEditData({...editData,country:e.target.value})} placeholder="Country" style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          <input className="mz-input" value={editData.state} onChange={e=>setEditData({...editData,state:e.target.value})} placeholder="State" style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          <input className="mz-input" value={editData.village} onChange={e=>setEditData({...editData,village:e.target.value})} placeholder="Village" style={{border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10, width:'100%'}}/>
          <div style={{display:'flex', gap:8, width:'100%'}}><input className="mz-input" value={editData.phone} onChange={e=>setEditData({...editData,phone:e.target.value})} placeholder="Phone" style={{flex:1, border:`1.5px solid ${border}`, background:bg2, color:text, borderRadius:10, padding:10}}/><label style={{display:'flex', alignItems:'center', gap:4, fontSize:12}}><input type="checkbox" checked={editData.phonePublic} onChange={e=>setEditData({...editData,phonePublic:e.target.checked})}/>Public</label></div>
          <div style={{display:'flex', gap:8, alignItems:'center', background:btnBg, padding:10, borderRadius:10, width:'100%'}}><span style={{flex:1, fontSize:13, color:text}}>Email: {email} (Can't edit)</span><label style={{display:'flex', alignItems:'center', gap:4, fontSize:12}}><input type="checkbox" checked={editData.emailPublic} onChange={e=>setEditData({...editData,emailPublic:e.target.checked})}/>Public</label></div>
          <div style={{display:'flex', gap:8, marginTop:6, width:'100%'}}><button onClick={()=>setShowEdit(false)} style={{flex:1, padding:12, borderRadius:12, border:'none', background:btnBg, color:text, fontWeight:700}}>Cancel</button><button onClick={saveEdit} style={{flex:1, padding:12, borderRadius:12, border:'none', background:'#7C3AED', color:'#fff', fontWeight:800}}>Save</button></div>
        </div>
      </div>
    </div>}
  </div>
