...
export default function ProfilePage(){
    ...
    const load=async()=>{
      ...
      try{
        ...
        if(snap.exists()){
          ...
          if(d.bio) setBio(d.bio); if(d.dob) setDob(d.dob); if(d.hobby) setHobby(d.hobby); if(d.games) setGames(d.games); if(d.phone) setPhone(d.phone)
          if(d.phonePublic!==undefined) setPhonePublic(d.phonePublic); if(d.emailPublic!==undefined) setEmailPublic(d.emailPublic)
          setEditData({bio:d.bio||'',dob:d.dob||'',hobby:d.hobby||'',games:d.games||'',phone:d.phone||'',phonePublic:d.phonePublic||false,emailPublic:d.emailPublic||false,name:d.name||localName,pic:d.photoURL||localPic})
        }
  ...
  const saveEdit=async()=>{
    setName(editData.name); setBio(editData.bio); setDob(editData.dob); setHobby(editData.hobby); setGames(editData.games); setPhone(editData.phone); setPhonePublic(editData.phonePublic); setEmailPublic(editData.emailPublic); if(editData.pic) setPic(editData.pic)
    localStorage.setItem('mz_user_name',editData.name); localStorage.setItem('mz_bio',editData.bio); localStorage.setItem('mz_dob',editData.dob); localStorage.setItem('mz_hobby',editData.hobby); localStorage.setItem('mz_games',editData.games); localStorage.setItem('mz_phone',editData.phone); localStorage.setItem('mz_phone_pub',editData.phonePublic?'1':'0'); localStorage.setItem('mz_email_pub',editData.emailPublic?'1':'0'); if(editData.pic) localStorage.setItem('mz_pic',editData.pic)
    const uid=auth.currentUser?.uid; if(uid) await setDoc(doc(db,"users",uid),{name:editData.name,bio:editData.bio,dob:editData.dob,hobby:editData.hobby,games:editData.games,phone:editData.phone,phonePublic:editData.phonePublic,emailPublic:editData.emailPublic,photoURL:editData.pic},{merge:true})
    setShowEdit(false)
  }

  return <div style={{minHeight:'100vh', background:'#fff', paddingBottom:80}}>
    {/* HEADER */}
    <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px 16px', borderBottom:'1px solid #eee', position:'sticky', top:0, background:'#fff', zIndex:20}}>
      <span style={{fontWeight:900, fontSize:22}}>{name}</span>
      <button onClick={()=>setShowMenu(!showMenu)} style={{fontSize:26, background:'none', border:'none'}}>☰</button>
    </div>

    {/* 1. HAMBURGER MENU - Thlalak 2 ang */}
    {showMenu && <>
      <div onClick={()=>setShowMenu(false)} style={{position:'fixed', inset:0, zIndex:40}}/>
      <div style={{position:'fixed', top:50, right:12, width:180, background:'#fff', borderRadius:16, boxShadow:'0 8px 30px rgba(0,0,0,0.15)', zIndex:50, overflow:'hidden'}}>
        <button onClick={()=>{setShowMenu(false); router.push('/settings')}} style={{width:'100%', display:'flex', alignItems:'center', gap:12, padding:'14px 16px', border:'none', background:'#fff', fontSize:16, fontWeight:500, cursor:'pointer'}}>
          <span style={{fontSize:20}}>⚙️</span> Setting
        </button>
        <button onClick={()=>{localStorage.setItem('mz_online','false'); localStorage.setItem('mz_logged_out','true'); localStorage.clear(); router.push('/')}} style={{width:'100%', display:'flex', alignItems:'center', gap:12, padding:'14px 16px', border:'none', background:'#fff', fontSize:16, fontWeight:500, color:'#ff3b30', cursor:'pointer'}}>
    ...
    <div style={{padding:16}}>
      {/* 3. HMING & PIC & STATS - Pic chip tawp nen inrual */}
      <div style={{display:'flex', gap:16, alignItems:'flex-start'}}>
        {/* PIC - Click View, + Change */}
        <div style={{position:'relative'}}>
          ...
          <div onClick={()=>fileRef.current?.click()} style={{position:'absolute', bottom:-2, right:-2, width:28, height:28, background:'#7C3AED', borderRadius:'50%', border:'2px solid #fff', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontWeight:900, cursor:'pointer'}}>+</div>
          <input ref={fileRef} type="file" accept="image/*" hidden onChange={handlePicChange}/>
        </div>

        {/* HMING CHUNG AH, POSTS etc CHUNG AH - Top aligned */}
        <div style={{flex:1}}>
          <div style={{fontWeight:800, fontSize:15, marginBottom:8}}>{name}</div>
      ...
      {/* 4. ABOUT - Bio, DOB, Hobby, Games, Phone, Email private/public */}
      <div style={{marginTop:16, background:'#F9F6FF', borderRadius:14, padding:12, display:'flex', flexDirection:'column', gap:8}}>
        <div style={{fontWeight:800, fontSize:14, display:'flex', gap:6}}>📝 <span>About</span></div>
        {bio && <div style={{fontSize:13}}>📖 Bio: {bio}</div>}
        {dob && <div style={{fontSize:13}}>🎂 DOB: {dob}</div>}
        {hobby && <div style={{fontSize:13}}>🎨 Hobby: {hobby}</div>}
        {games && <div style={{fontSize:13}}>🎮 Games: {games}</div>}
        {phone && (phonePublic? <div style={{fontSize:13}}>📞 Phone: {phone} 🌍</div> : <div style={{fontSize:13}}>📞 Phone: •••••• 🔒 (Private)</div>)}
        <div style={{fontSize:13}}>{emailPublic? `📧 Email: ${email} 🌍` : `📧 Email: •••••• 🔒 (Private)`}</div>
      </div>

      <div style={{display:'flex', gap:8, marginTop:12}}>
        <button onClick={()=>setShowEdit(true)} style={{flex:1, padding:'9px', borderRadius:10, background:'#efefef', border:'none', fontWeight:800}}>Edit profile</button>
        <button onClick={async()=>{const link=`https://mz-apps-mauve.vercel.app/user/${auth.currentUser?.uid}`; if(navigator.share) await navigator.share({title:name, url:link}); else {await navigator.clipboard.writeText(link); alert('Copied!')}}} style={{flex:1, padding:'9px', borderRadius:10, background:'#efefef', border:'none', fontWeight:800}}>Share profile</button>
      </div>
    </div>

    {/* POSTS */}
    <div ref={postsRef} style={{borderTop:'1px solid #eee', marginTop:6}}>
      <div style={{display:'flex', justifyContent:'center', padding:'12px 0'}}><span style={{fontWeight:900, borderBottom:'2px solid #000', paddingBottom:4}}>⊞ POSTS</span></div>
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:2}}>
        {posts.length===0? [1,2,3].map(i=><div key={i} style={{aspectRatio:'1', background:'#fafafa', display:'flex', alignItems:'center', justifyContent:'center', color:'#ccc', fontSize:12}}>No posts</div>) : posts.map((p,i)=><div key={i} style={{aspectRatio:'1', background:`url(${p.image}) center/cover #eee`}}/>)}
      </div>
    </div>
    ...
    {/* EDIT MODAL - About fields edit theih, email edit theih lo */}
