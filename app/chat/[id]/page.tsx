    ...
    <div style={{height:'100dvh', display:'flex', flexDirection:'column', background: theme==='dark'?'#0b141a':'#efeae2', position:'relative'}}>
      ...
      {isSearchMode && (
        <div style={{background: '#202c33', padding:'8px 10px', display:'flex', gap:8, alignItems:'center'}}>
          <input value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} autoFocus placeholder="Search in chat..." style={{flex:1, background:'#2a3942', border:'none', borderRadius:20, padding:'8px 14px', outline:'none', color:'#fff'}}/>
          <button onClick={()=>{ setIsSearchMode(false); setSearchQuery('') }} style={{border:'none', background:'none', fontWeight:700, color:'#00a884', cursor:'pointer'}}>Cancel</button>
        </div>
      )}
      ...
      <div ref={listRef} style={{
        ...
        justifyContent: filteredMessages.length < 8? 'flex-end' : 'flex-start'
      }}>
        <div>
            ...
            return (
              <div key={m.id} style={{display:'flex', justifyContent: isMe? 'flex-end' : 'flex-start', marginBottom:6}}>
                <div style={{
                  ...
                  padding: m.imageUrl? '4px 4px 4px 4px' : '6px 8px 4px 10px',
                  boxShadow:'0 1px 1px rgba(0,0,0,0.2)',
                  display:'flex', flexDirection:'column'
                }}>
                  {m.imageUrl && <img src={m.imageUrl} style={{maxWidth:260, borderRadius:8, marginBottom: m.text? 4:0}}/>}
