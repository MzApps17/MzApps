...
export default function ChatPage() {
    ...
    const unsubAuth = onAuthStateChanged(auth, async (u)=>{
      ...
      try {
        ...
        if(otherDoc.exists()) {
          ...
          setOtherUser(data)
          setIsOtherOnline(data.isOnline || false)
          localStorage.setItem('mz_view_user', JSON.stringify(data))
        }
      } catch {}
  ...
  const handleBlock = async () => {
    ...
    try {
      await updateDoc(doc(db, 'users', myUid), { blockedUsers: arrayUnion(id) })
      alert('Blocked')
      router.push('/chat')
    } catch {
  ...
  const Tick = ({ status }: { status: string }) => {
    ...
    if(status === 'seen') return <span style={{fontSize:11, color:'#53BDEB', marginLeft:4, letterSpacing:'-3px', fontWeight:900}}>✓✓</span>
    return null
  }
  ...
  return (
    <div style={{height:'100dvh', display:'flex', flexDirection:'column', background:'#0b141a', position:'relative'}}>
      ...
      <div style={{ position:'fixed', bottom:68, left:0, right:0, background: '#202c33', padding:'6px 8px', display:'flex', gap:8, alignItems:'center', zIndex:25 }}>
        ...
        <button onClick={()=>fileInputRef.current?.click()} style={{width:46, height:46, borderRadius:23, background:'#2a3942', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center'}}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#8696a0" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
        </button>
        <button onClick={()=>handleSend()} disabled={!input.trim() || uploading} style={{width:46, height:46, borderRadius:23, background:'#00a884', border:'none', color:'white', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', opacity: input.trim()? 1 : 0.6}}>
